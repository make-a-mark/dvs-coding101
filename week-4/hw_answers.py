#Question 1: Cut the following word in half in whichever way you want.
full_word = "DVS Coding 101"
first_half = full_word[0:8]
second_half = full_word[8:14]

#Question 2: Check that first_half and second_half are the same length (Use an operation, it should return true)
check_len = len(first_half) == len(second_half)

#Question 3: Use the "replace" string method to turn the word "Lemon" into "Leon" (https://www.tutorialspoint.com/python/string_replace.htm)
word = "Lemon"
new_word = word.replace("m", "")
print(new_word)

#Question 4: Use the "replace" string method to correct our_word to the string "Python" (there are multiple ways to do this)
our_word = "Pytpon"
fixed_word = our_word.replace("pon", "hon")


#Question 5: Count the number of "l"s and "o"s combined
count_lo = "Hello World"
total_lo = count_lo.count("l") + count_lo.count("o")

#Question 6: Assign the string "weather" to the variable word_1 using string slicing.
phrase_1 = "How's the weather today?"
word_1 = phrase_1[10: 17]

#Question 7: Fill and fix the list which have the days of the week in it.
#Take as many lines as you need.
days = ["Monday", "Tuesday", "Wednesday", "Fridank", "racecar", "Saturday"]
days.remove("racecar")
days.append("Sunday")
days.insert(3, "Thursday")
days[4] = "Friday"



#Question 8: What is the largest index of days after being fixed? (Use functions/methods and arithmetic to calculate this)
largest_index = len(days) - 1